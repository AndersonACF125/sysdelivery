...
$dispmoto 	= $connect->query("SELECT * FROM motoboy WHERE id='$cod_id'");
...
$msg4 =  "Pedido para entrega! Fvor coletar o pedido.\n";
$msg4 .= "\n";
$msg4 .= "*".$nomeempresa."*\n";
$msg4;

if(isset($_POST["dispmoto"]))  {
$update = $connect->query("UPDATE motoboy SET status='5' WHERE id='".$_POST["dispmoto"]."'");
header("location: pdv.php?ok="); 
}
		  ...
		  <li class="nav-item">
            <a class="nav-link" href="#">
			  ...
			  <?php
				...
				if($pedido->status == 5){print $statusxx = "Status Atual - Disp Motoboy";}
		  ...
		  <div class="col-md-6">
			<div class="card card-people-list pd-15 mg-b-10">
				...
				<div class="row">
					...
					<div class="col-md-4 mg-b-10">
    <button type="button" class="btn btn-success btn-block" onclick="openModal()">Disp. Motoboy</button>
</div>	
<div id="motoboyModal" class="modal">
  <div class="modal-content">
    <form id="motoboyForm" action="" method="post">
      <input type="hidden" name="disparamoto" value="<?=$codigop;?>">
      <input type="hidden" id="whatsapp" value="<?=$celcli;?>">
      <input type="hidden" id="mensagem6" value="<?=$msg2; ?>">
      <div class="form-group">
        <label for="motoboySelect">Motoboy</label>
        <select id="cad_nome" name="motoboy" required><p>
          <option value="">Selecione...</option>
          <?php
          // Loop através do array de motoboys e crie as opções do select
          foreach ($moto as $motoboy) {
...
<script language="javascript">
   ...
   docprint.document.write('"http://www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd">');
   docprint.document.write('<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">');
   docprint.document.write('<head><title>COMANDA COZINHA</title>');
   docprint.document.write('<style type="text/css">body{ margin:0px;');
   docprint.document.write('font-family:verdana,Arial;color:#000;');
   docprint.document.write('font-family:Verdana, Geneva, sans-serif; font-size:12px;}');
   docprint.document.write('a{color:#000;text-decoration:none;} </style>');
...
function enviarMensagem6(){
	var celular = document.querySelector("#whatsapp").value;
    whatsapp = whatsapp.replace(/\D/g,'');
	  if(whatsapp.length < 13){
		whatsapp = "55" + whatsapp;
	  }
	  var texto = document.querySelector("#mensagem6").value;
	  texto = window.encodeURIComponent(texto);
