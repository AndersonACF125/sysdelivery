...
// Verifica o estado do sino a partir do cookie
$bellStatus = isset($_COOKIE['bellStatus']) ? $_COOKIE['bellStatus'] : 'on'; // Padrão é ligado

// Se o formulário foi submetido, atualize o estado do sino no cookie
if (isset($_POST['bellToggle'])) {
    $bellStatus = $_POST['bellToggle'];
    setcookie('bellStatus', $bellStatus, time() + (86400 * 30), "/"); // 86400 = 1 dia
}
?>
<!DOCTYPE html>
<html lang="en">
      ...
      <div class="container">
        <ul class="nav">
          <li class="nav-item">
           ...
           <!-- Sino e Botão de Controle de Som -->
           <li class="nav-item">
              <a class="nav-link" href="#">
              <div>
  <input type="range" id="volume" name="volume" min="0" max="11" />
  <label for="volume"> Volume Som</label>
</div>
              </a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="#">
              <span>
                PROGRESSO <progress value="0" max="30" id="progressBar"></progress>
              </span>
            </a>
          </li>
          <li class="nav-item">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var bellToggle = document.getElementById('bellToggle');
        var bellForm = document.getElementById('bellForm');
        var audio = new Audio('campainha.mp3');

        bellToggle.addEventListener('change', function () {
            if (bellToggle.checked) {
                audio.play();
            } else {
                audio.pause();
                audio.currentTime = 0;
            }
            bellForm.submit(); // Atualiza o estado do sino no servidor
        });
    });
</script>
