...
<!-- Seção para inserir o código do cupom de desconto -->
<center class="mg-b-5"><span class="tx-14">Insira o código do cupom de desconto:</span></center>
<div class="slim-card-title mg-b-15">
    <form id="cupomForm" method="post" action="aplicar_cupom.php">
        <div class="form-group">
            <input type="text" name="cupom" id="cupom" class="form-control" placeholder="Código do cupom" required>
        </div>
        <button type="submit" class="btn btn-primary btn-block">Aplicar Cupom</button>
    </form>
</div>
...
if(isset($_POST['cupom'])) {
    ...
    $verificaCupom = $connect->query("SELECT * FROM cupons WHERE codigo = '$cupom' AND validade >= NOW()");
    if ($verificaCupom->rowCount() > 0) {
        $cupomData = $verificaCupom->fetch(PDO::FETCH_OBJ);
        $desconto = $cupomData->desconto;
        // Aplica o desconto
        // Aqui você deve ajustar o valor total com base no desconto
        $valorTotal = $somando->soma;
        $valorTotalComDesconto = $valorTotal - ($valorTotal * ($desconto / 100));
    } else {
        $valorTotalComDesconto = $somando->soma;
        echo '<div class="alert alert-danger">Cupom inválido ou expirado.</div>';
    }
} else {
    $valorTotalComDesconto = $somando->soma;
}
    ...
    <div class="card card-people-list pd-15" style="background-color:<?php print $dadosempresa->corcarrinho; ?>">
        <div class="media-list">
            ...
            <div class="row mg-t-10">
                ...
                <div class="col-5"><strong>R$: <?php echo number_format($valorTotalComDesconto, 2, ',', '.'); ?></strong></div>
            </div>
            
            <?php if($valorTotalComDesconto > $dadosempresa->dfree) { ?>
            <hr>
